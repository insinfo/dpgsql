...
/// Represents a source of data for Npgsql, which can be used to create
/// connections. Handles connection pooling when `Pooling=true`.
class NpgsqlDataSource {
  NpgsqlDataSource(this.connectionString)
      : _builder = NpgsqlConnectionStringBuilder(connectionString) {
  ...
  final NpgsqlConnectionStringBuilder _builder;
  final Queue<_PooledConnector> _idleConnectors = Queue<_PooledConnector>();
  final Queue<_PoolWaiter> _waiters = Queue<_PoolWaiter>();
  final Map<NpgsqlConnector, _PooledConnector> _connectors =
      <NpgsqlConnector, _PooledConnector>{};
  ...
  Future<NpgsqlConnection> openConnection() async {
    ...
    if (!pooling) {
      ...
      return NpgsqlConnection.fromConnector(connector, null);
    }
    ...
    while (true) {
      ...
      try {
        ...
        return NpgsqlConnection.fromConnector(
          pooled.connector,
          _returnConnector,
        );
      } catch (_) {
...
class _PooledConnector {
  ...
  final NpgsqlConnector connector;
