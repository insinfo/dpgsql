...
import 'dpgsql_large_object_stream.dart';

/// Large object manager. This class can be used to store very large files in a PostgreSQL database.
/// Porting DpgsqlLargeObjectManager.cs
class DpgsqlLargeObjectManager {
  DpgsqlLargeObjectManager(this._connection);
  ...
  Future<DpgsqlLargeObjectStream> openRead(int oid) async {
    return _open(oid, _INV_READ);
  }

  /// Opens a large object on the backend, returning a stream controlling this remote object.
  /// Note that this method, as well as operations on the stream must be wrapped inside a transaction.
  Future<DpgsqlLargeObjectStream> openReadWrite(int oid) async {
    return _open(oid, _INV_READ | _INV_WRITE);
  }

  Future<DpgsqlLargeObjectStream> _open(int oid, int mode) async {
    final fd = await _executeFunction('lo_open', [oid, mode]);
    return DpgsqlLargeObjectStream(this, fd, (mode & _INV_WRITE) != 0);
  }
  ...
  // Internal methods for DpgsqlLargeObjectStream
